...
class ApiController extends Controller
{
    ...
    public function send_plan_stu(Request $request)
    {
        ...
        if ($mosh) {
            ...
            if ($request->file) {
                ...
                $request->file->move(public_path() .'/'. $imagePath, $imageName);
    ...
    public function detail_report(Request $request)
    {
        ...
        foreach ($days_part as $key2 => $value2) {
            ...
            if ($lesson->status == 1) {
                if (!in_array($lesson->title, $lessonEids)) {
                    $arrayExclusive[$lesson->title][0] = 0;
                ...
                if ($value2->study_time && strpos($value2->study_time, ':')) {
                    ...
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->study_time)[0] * 3600;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->study_time)[1] * 60;
                    // $timeStudy_E += 
                } else {
                    $SumExclusive +=  $value2->study_time * 3600;
                    $arrayExclusive[$lesson->title][0] += $value2->study_time * 3600;
                }
                if ($value2->test_time && strpos($value2->test_time, ':')) {
                    $SumExclusive +=  explode(':', $value2->test_time)[0] * 3600;
                    $SumExclusive +=  explode(':', $value2->test_time)[1] * 60;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->test_time)[0] * 3600;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->test_time)[1] * 60;
                    $testTime_E += (explode(':', $value2->test_time)[0] * 3600) + (explode(':', $value2->test_time)[1] * 60);
                } else {
                    $SumExclusive +=  $value2->test_time * 3600;
                    $arrayExclusive[$lesson->title][0] += $value2->test_time * 3600;
                    $testTime_E += $value2->test_time * 3600;
                }
                if ($value2->Pre_reading && strpos($value2->Pre_reading, ':')) {
                    $SumExclusive += explode(':', $value2->Pre_reading)[0] * 3600;
                    $SumExclusive += explode(':', $value2->Pre_reading)[1] * 60;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->Pre_reading)[0] * 3600;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->Pre_reading)[1] * 60;
                } else {
                    $SumExclusive += $value2->Pre_reading * 3600;
                    $arrayExclusive[$lesson->title][0] += $value2->Pre_reading * 3600;
                }
                if ($value2->exercise && strpos($value2->exercise, ':')) {
                    $SumExclusive += explode(':', $value2->exercise)[0] * 3600;
                    $SumExclusive += explode(':', $value2->exercise)[1] * 60;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->exercise)[0] * 3600;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->exercise)[1] * 60;
                } else {
                    $SumExclusive += $value2->exercise * 3600;
                    $arrayExclusive[$lesson->title][0] += $value2->exercise * 3600;
                }
                if ($value2->Summarizing && strpos($value2->Summarizing, ':')) {
                    $SumExclusive += explode(':', $value2->Summarizing)[0] * 3600;
                    $SumExclusive += explode(':', $value2->Summarizing)[1] * 60;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->Summarizing)[0] * 3600;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->Summarizing)[1] * 60;
                } else {
                    $SumExclusive += $value2->Summarizing * 3600;
                    $arrayExclusive[$lesson->title][0] += $value2->Summarizing * 3600;
                }
                if ($value2->passage && strpos($value2->passage, ':')) {
                    $SumExclusive += explode(':', $value2->passage)[0] * 3600;
                    $SumExclusive += explode(':', $value2->passage)[1] * 60;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->passage)[0] * 3600;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->passage)[1] * 60;
                } else {
                    $SumExclusive += $value2->passage * 3600;
                    $arrayExclusive[$lesson->title][0] += $value2->passage * 3600;
                }
                if ($value2->Repeat_test && strpos($value2->Repeat_test, ':')) {
                    $SumExclusive += explode(':', $value2->Repeat_test)[0] * 3600;
                    $SumExclusive += explode(':', $value2->Repeat_test)[1] * 60;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->Repeat_test)[0] * 3600;
                    $arrayExclusive[$lesson->title][0] += explode(':', $value2->Repeat_test)[1] * 60;
                } else {
                    $SumExclusive += $value2->Repeat_test * 3600;
                    $arrayExclusive[$lesson->title][0] += $value2->Repeat_test * 3600;
                }
            } else {
                ...
                if ($value2->test_time && strpos($value2->test_time, ':')) {
                    ...
                    $testTime_G += (explode(':', $value2->test_time)[0] * 3600) + (explode(':', $value2->test_time)[1] * 60);
                } else {
                    $arrayGeneral[$lesson->title][0] += $value2->test_time * 3600;
                    $testTime_G += $value2->test_time * 3600;
                }
                if ($value2->study_time && strpos($value2->study_time, ':')) {
        ...
        $timeStudy_G = Tools::convertSecond_2_hours($SumGeneral - $testTime_G);
        ...
        $timeStudy_E = Tools::convertSecond_2_hours($SumExclusive - $testTime_E);
        ...
        $jsonArrayExclusive = [];
        foreach ($arrayExclusive as $key => $value) {
            array_push($jsonArrayExclusive,[
                'title' => $key,
        ...
        return response()->json([
            'sum' => $sum, 
            'SumExclusive' => $SumExclusive, 
            'exclusive' => [
                'studyTime' => $timeStudy_E,
                'testTime'  => Tools::convertSecond_2_hours($testTime_E),
            ], 
            'SumGeneral' => $SumGeneral, 
            'gneral' => [
                'studyTime' => $timeStudy_G,
                'testTime'  => Tools::convertSecond_2_hours($testTime_G),
            ],
            'jsonArrayGeneral' => $jsonArrayGeneral,
            'jsonArrayExclusive' => $jsonArrayExclusive,
    ...
    public function chart_report(Request $request, $stu_id, $week_id)
    {
        $stu = DB::table('stu')->where('id',$stu_id)->first();
    ...
    {
        return response()->json(
            ['status' => 0, 'title' => 'برنامه هفتگی', 'price' => 15000]
        );
        $stu_id = explode(';', $request->token)[1];
        $stu = Stu::where('id', $stu_id)->first();
    ...
    public function get_chat(Request $request)
    {
        ...
        foreach ($chats as $key => $value) {
            ...
            if($value->replay_to)
                $chats[$key]->replay_message = Tools::findReplayFrom($chats,$value->replay_to);
            else
                $chats[$key]->replay_message = null;
        }
    ...
    public function send_chat(Request $request)
    {
        ...
        $chat->date_time = time();
        $chat->current = 1;
        $chat->replay_to = $request->replay_to;
        if ($chat->save()) {
            if ($request->file) {
    ...
    public function get_stu_mosh(Request $request)
    {
        ...
        foreach ($all_week as $key => $value) {
            ...
            $n = Verta::createTimestamp((int)$value->start);
            $all_week[$key]->start = $n->format('Y-n-j');
        }
    ...
    public function show_pv(Request $request)
    {
        ...
        foreach ($all_message as $key => $value) {
            ...
            if($value->replay_to)
                $all_message[$key]->replay_message = Tools::findReplayFrom($all_message,$value->replay_to);
            else
                $all_message[$key]->replay_message = null;
        }
    ...
    public function create_plan(Request $request){
        ...
        // check data or image file sended
        if ($data) {
            $date = Verta::createTimestamp(time());
            $date = $date->formatJalaliDate();
            $data = [
        ...
        // create history planning
        DB::table('history_planing')->insert([
            'stu_id' => $stu_id,
            'planing_id' => 3,
            'mosh_id' => $mosh_id,
            'addr' => '/planfiles/' .$filename,
            'time_added' => time(),
            'ready' => 1,
            'status' => 1,
        ]);

        // send new massage for student
        $title = "برنامت آمادس!!";
        $text = "برای دریافت برنامه به بخش برنامه های من برو!!";
        $notification = new FirebaseController;
        $notification->spnMosh($stu->FirebaseToken, $title, $text);

        return response()->json(['success' => true ,'link'=>'/planfiles/' . $filename]);
    }
